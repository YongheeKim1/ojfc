...
type Screen = 'welcome' | 'main' | 'register' | 'existing';

export default function LoginPage({ onLogin }: { onLogin: () => void }) {
  const [screen, setScreen] = useState<Screen>('main');
  const [name, setName] = useState('');
  const [password, setPassword] = useState(() => localStorage.getItem('ojifc_savedPw') || '');
  const [selectedPositions, setSelectedPositions] = useState<Position[]>([]);
  const [members, setMembers] = useState<Member[]>(getMembers());
  const [currentUser, setCurrentUserState] = useState<Member | null>(null);
  const [loginName, setLoginName] = useState(() => localStorage.getItem('ojifc_savedName') || '');
  const [loginError, setLoginError] = useState('');
  const [rememberMe, setRememberMe] = useState(() => !!localStorage.getItem('ojifc_savedName'));
  ...
  const handleExistingLogin = (e: React.FormEvent) => {
    ...
    if (rememberMe) {
      localStorage.setItem('ojifc_savedName', trimmedName);
      localStorage.setItem('ojifc_savedPw', password);
    } else {
      localStorage.removeItem('ojifc_savedName');
      localStorage.removeItem('ojifc_savedPw');
    }
    setCurrentUser(member);
    onLogin();
  };
  ...
  // Existing member login (이름 + 비밀번호 폼)
  if (screen === 'existing') {
    return (
      <div className="min-h-screen bg-[#1e3a5f] flex flex-col items-center justify-center px-6">
        <div className="w-full max-w-sm flex flex-col items-center">
          ...
          <p className="text-sm text-blue-200 mb-6">이름과 비밀번호를 입력하세요</p>

          <form onSubmit={handleExistingLogin} className="w-full space-y-3" autoComplete="on">
            <div>
              <input
                type="text"
                name="username"
                ...
                placeholder="이름"
                maxLength={10}
                className="w-full px-4 py-3.5 bg-white/10 border border-white/20 rounded-xl text-sm text-white placeholder-blue-300/50 outline-none focus:ring-2 focus:ring-[#16a34a]/50 focus:border-[#16a34a] transition"
                autoFocus
              />
            </div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="비밀번호"
                className="w-full pl-11 pr-4 py-3.5 bg-white/10 border border-white/20 rounded-xl text-sm text-white placeholder-blue-300/50 outline-none focus:ring-2 focus:ring-[#16a34a]/50 focus:border-[#16a34a] transition"
              />
            </div>
            ...
            <label className="flex items-center gap-2 cursor-pointer py-1">
              <input
                ...
                className="w-4 h-4 rounded border-white/30 bg-white/10 text-[#16a34a] focus:ring-[#16a34a]/50"
              />
              <span className="text-xs text-blue-200">로그인 정보 저장</span>
            </label>
            ...
            <button
              type="submit"
              disabled={!loginName.trim() || !password.trim()}
            ...
            <button
              type="button"
              onClick={() => { setScreen('main'); setLoginName(''); setPassword(''); setLoginError(''); }}
